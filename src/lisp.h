...
/* Boxbot external functions */
extern void setup_move(bool dir, int dist);
extern void setup_turn(bool dir, int angle);
...
FILE *input_from_buffer(const char *s) {
  return fin <= 9 && (in[fin] = fmemopen((void *)s, strlen(s), "r")) ? in[fin++] : NULL;
}
...
L f_boxbot_move(L t, L *_) {
  int dist;
  dist = car(t);
  setup_move(dist > 0, dist < 0 ? -dist : dist);
  return nil;
}
